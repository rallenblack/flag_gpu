...
int main(int argc, char *argv[]) {
	...
	int genFlag = 0;
	if(argc <= optind) {
		//(void) fprintf(stderr, "ERROR: Missing data file so data is generated.\n");
		(void) fprintf(stdout, "INFO: Missing data file so data is generated.\n");
		genFlag = 1;
	}
	...
	ret = resetDevice();
        ...
        //FILE * file = fopen("outfile.dat", "wb");
        //fwrite(g_outputData, outputSize, 1, file);
        //fclose(file);
