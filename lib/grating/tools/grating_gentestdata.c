...
#define DEF_NUM_SAMPS   1024      /* 10M samples */
#define F_S             (155.0/512) /* sampling frequency in MHz */
#define NUM_FREQS       1
#define NUM_SUBBANDS    200
...
int main(int argc, char *argv[])
{
    ...
    float afFreqX[NUM_FREQS][NUM_SUBBANDS]; /* in MHz */
    float afFreqY[NUM_FREQS][NUM_SUBBANDS]; /* in MHz */
    float afAmplX[NUM_FREQS][NUM_SUBBANDS];
    float afAmplY[NUM_FREQS][NUM_SUBBANDS];
    ...
    /* Populate afFreqX, afFreqY, afAmplX, and afAmplY */
    for (s = 0; s < NUM_SUBBANDS; s++) {
        for (f = 0; f < NUM_FREQS; f++) {
            i = f*NUM_SUBBANDS + s;
            afFreqX[f][s] = (i+1.0)*F_S/(NUM_SUBBANDS*NUM_FREQS+1.0);
            afFreqY[f][s] = (i+1.0)*F_S/(NUM_SUBBANDS*NUM_FREQS+1.0);
            afAmplX[f][s] = 0.01;
            afAmplY[f][s] = 0.0;
        }
    }
    ...
    for (i = 0; i < iLen; ++i)
    {
        ...
        for (s = 0; s < NUM_SUBBANDS; ++s) {
            /* add some noise */
            cDataReX = 0*SCALE_FACTOR * ((0.6 * ((double) rand()) / RAND_MAX) - 0.3);
            cDataImX = 0*SCALE_FACTOR * ((0.6 * ((double) rand()) / RAND_MAX) - 0.3);
            /* add some signal */
            for (j = 0; j < NUM_FREQS; ++j) {
                /* NOTE: make sure that the sum is within [-128:127] */
                cDataReX += SCALE_FACTOR
                            * (afAmplX[j][s] * cos(2 * M_PI * afFreqX[j][s] * i / F_S));
                cDataImX += SCALE_FACTOR
                            * (afAmplX[j][s] * sin(2 * M_PI * afFreqX[j][s] * i / F_S));
            }
            ...
            /* generate data - pol Y */
            /* add some noise */
            cDataReY = 0*SCALE_FACTOR * ((0.2 * ((double) rand()) / RAND_MAX) - 0.1);
            cDataImY = 0*SCALE_FACTOR * ((0.2 * ((double) rand()) / RAND_MAX) - 0.1);
            /* add some signal */
            for (j = 0; j < NUM_FREQS; ++j) {
                /* NOTE: make sure that the sum is within [-128:127] */
                cDataReY += SCALE_FACTOR
                            * (afAmplY[j][s] * cos(2 * M_PI * afFreqY[j][s] * i / F_S));
                cDataImY += SCALE_FACTOR
                            * (afAmplY[j][s] * sin(2 * M_PI * afFreqY[j][s] * i / F_S));
            }
