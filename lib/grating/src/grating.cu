...
float4* g_pf4FFTIn = NULL;
float4* g_pf4FFTOut_d = NULL;
float4* g_pf4FFTOut = NULL;
...
int main(int argc, char *argv[])
{
    ...
    iRichardFile = open("myfile.dat",
                    O_CREAT | O_TRUNC | O_WRONLY,
                    S_IRUSR | S_IWUSR | S_IRGRP | S_IROTH);
    if (iRichardFile < EXIT_SUCCESS) {
        (void) fprintf(stderr, "ERROR! Opening Richard file failed!\n");
        CleanUp();
        return EXIT_FAILURE;
    }

    iBlackFile = open("fft_in.dat",
                    O_CREAT | O_TRUNC | O_WRONLY,
                    S_IRUSR | S_IWUSR | S_IRGRP | S_IROTH);
    if (iRichardFile < EXIT_SUCCESS) {
        (void) fprintf(stderr, "ERROR! Opening Black file failed!\n");
        CleanUp();
        return EXIT_FAILURE;
    }
#endif
...
#if OUTFILE
            cudaMemcpy(g_pf4FFTIn, g_pf4FFTIn_d,
                       (g_iNumSubBands*g_iNFFT*sizeof(float4)),
                       cudaMemcpyDeviceToHost);
            (void) write(iBlackFile, g_pf4FFTIn, g_iNFFT*g_iNumSubBands*sizeof(float4));
...
#if OUTFILE
        /* Dump raw frequency channels to file for Richard */
        CUDASafeCallWithCleanUp(cudaMemcpy(g_pf4FFTOut, g_pf4FFTOut_d,
                                           (g_iNFFT*g_iNumSubBands*sizeof(float4)),
                                           cudaMemcpyDeviceToHost));

        (void) write(iRichardFile, g_pf4FFTOut, g_iNFFT*g_iNumSubBands*sizeof(float4));
#endif
...
#if OUTFILE
    (void) close(iFileSpec);
    (void) close(iRichardFile);
    (void) close(iBlackFile);
#endif
...
int Init()
{
    ...
    if (g_iNFFT < g_iMaxThreadsPerBlock)
    {
        ...
        //g_dimBPFB.x = g_iNumSubBands;
        //g_dimBCopy.x = g_iNumSubBands;
        //g_dimBAccum.x = g_iNumSubBands;
    }
    else
    {
...
#if OUTFILE
    g_pf4FFTOut = (float4 *) malloc(g_iNumSubBands * g_iNFFT * sizeof(float4));
    g_pf4FFTIn  = (float4 *) malloc(g_iNumSubBands * g_iNFFT * sizeof(float4));
#endif
