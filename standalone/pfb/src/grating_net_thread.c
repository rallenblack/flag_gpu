...
static inline void initialize_block(grating_input_databuf_t * db, uint64_t mcnt) {
...
static void set_block_filled(grating_input_databuf_t * db, block_info_t * binfo) {
    ...
    grating_input_databuf_set_filled(db, block_idx);
...
static inline uint64_t process_packet(grating_input_databuf_t * db, struct hashpipe_udp_packet *p) {
    ...
    if (pkt_mcnt_dist >= 2*Nm) { // 2nd next block (Current block + 2)
        ...
        grating_input_databuf_wait_free(db, dest_block_idx);
    ...
    uint64_t * dest_p  = db->block[dest_block_idx].data + grating_input_databuf_idx(binfo.m, binfo.f, 0, 0);
...
static void *run(hashpipe_thread_args_t * args) {
    ...
    grating_input_databuf_t *db = (grating_input_databuf_t *)args->obuf;
    ...
    */
    ...
    for (i = 0; i < 2; i++) {
        ...
        if (grating_input_databuf_wait_free(db, i) != HASHPIPE_OK) {
...
static hashpipe_thread_desc_t net_thread = {
    name: "grating_net_thread",
    skey: "NETSTAT",
    init: NULL,
    run:  run,
    ibuf_desc: {NULL},
    obuf_desc: {grating_input_databuf_create}
};
