...
def start_time_callback(p):
    ...
    mjd = p.val_struct[0].val_struct[1].val_int64[0]
    #print "mjd:", mjd
    ...
    if mjd > 0 and seconds > 0:
        print "Start time has been set by scan coordinator!"
        start_time = datetime.utcfromtimestamp(dmjd_2_secs(mjd + fraction)).replace(tzinfo=pytz.utc)
        print "Passing start time:", start_time, "to Dealer.start"
        D.start(start_time)
